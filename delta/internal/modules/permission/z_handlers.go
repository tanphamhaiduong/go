...
func (h *HandlerImpl) GetByID(ctx context.Context, params arguments.PermissionGetByIDArgs) (models.Permission, error) {
	log.WithFields(log.Fields{
		...
		"params":  params,
	}).Info("Handler GetByID of permission")
	var (
	...
	if err := validator.Struct(params); err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler GetByID validator.Struct error of permission")
	...
	if err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler GetByID h.permission.GetByID error of permission")
		return permission, err
	}
...
func (h *HandlerImpl) GetByIDs(ctx context.Context, params arguments.PermissionGetByIDsArgs) ([]models.Permission, error) {
	log.WithFields(log.Fields{
		"TraceID": ctx.Value("TraceID"),
		"params":  params,
	}).Info("Handler GetByIDs of permission")
	var (
	...
	if err := validator.Struct(params); err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler GetByIDs validator.Struct error of permission")
	...
	if err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler GetByIDs h.permission.GetByIDs error of permission")
		return permissions, err
	}
...
func (h *HandlerImpl) Count(ctx context.Context, params arguments.PermissionCountArgs) (int64, error) {
	log.WithFields(log.Fields{
		"TraceID": ctx.Value("TraceID"),
		"params":  params,
	}).Info("Handler Count of permission")
	var (
		count int64
	)
	if err := validator.Struct(params); err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler Count validator.Struct error of permission")
		return count, err
	}
	...
	if err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler Count h.permission.Count error of permission")
		return count, err
	}
	return count, nil
}
...
func (h *HandlerImpl) List(ctx context.Context, params arguments.PermissionListArgs) ([]models.Permission, error) {
	log.WithFields(log.Fields{
		"TraceID": ctx.Value("TraceID"),
		"params":  params,
	}).Info("Handler List of permission")
	var (
		permissions []models.Permission
	)
	if err := validator.Struct(params); err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler List validator.Struct error of permission")
		return permissions, err
	}
	...
	if err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler List h.permission.List error of permission")
		return permissions, err
	}
	return permissions, nil
}
...
func (h *HandlerImpl) Insert(ctx context.Context, params arguments.PermissionInsertArgs) (models.Permission, error) {
	log.WithFields(log.Fields{
		"TraceID": ctx.Value("TraceID"),
		"params":  params,
	}).Info("Handler Insert of permission")
	var (
		permission models.Permission
	)
	if err := validator.Struct(params); err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler Insert validator.Struct error of permission")
		return permission, err
	}
	...
	if err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler Insert h.permission.Insert error of permission")
		return permission, err
	}
	return permission, nil
}
...
func (h *HandlerImpl) Update(ctx context.Context, params arguments.PermissionUpdateArgs) (models.Permission, error) {
	log.WithFields(log.Fields{
		"TraceID": ctx.Value("TraceID"),
		"params":  params,
	}).Info("Handler Update of permission")
	var (
		permission models.Permission
	)
	if err := validator.Struct(params); err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler Update validator.Struct error of permission")
		return permission, err
	}
	...
	if err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler Update h.permission.Update error of permission")
		return permission, err
	}
	return permission, nil
}
...
func (h *HandlerImpl) Delete(ctx context.Context, params arguments.PermissionDeleteArgs) (int64, error) {
	log.WithFields(log.Fields{
		"TraceID": ctx.Value("TraceID"),
		"params":  params,
	}).Info("Handler Delete of permission")
	var (
		id int64
	)
	if err := validator.Struct(params); err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler Delete validator.Struct error of permission")
		return id, err
	}
	...
	if err != nil {
		log.WithFields(log.Fields{
			"TraceID": ctx.Value("TraceID"),
			"Error":   err,
		}).Error("Handler Delete h.permission.Delete error of permission")
		return id, err
	}
	return id, nil
}
